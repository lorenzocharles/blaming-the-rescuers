...
2. In Terminal, navigate to the repository's local clone, then `npm install`. This will install all the required modules.  
3. Once all the modules are installed, `grunt`!
4. Grunt will rebuild the site (blog, sass, everything!) and serve it at `http://localhost:3000`.
...
1. Log in to [CloudFlare](https://www.cloudflare.com/a/login) (CF)
2. Add domain `blamingtherescuers.org`
3. Get CF nameservers (e.g. `owen.ns.cloudflare.com`) and set them up in the domain registrar admin page
4. Back to CF, in *Crypto* change SSL to `Flexible`
5. In *DNS* create two `A` records and point them to GitHub's IP addresses 

	Type | Name | Value
	---- | ---- | -----
	`A` | `blamingtherescuers.org` | `192.30.252.153` 
	`A` | `blamingtherescuers.org` | `192.30.252.154` 
6. Click on the cloud icon next to the two new `A` records, to switch it from grey to orange.
7. In GH add/edit `CNAME` in the root directory of this repository and type in `blamingtherescuers.org`	

<!--8. In `_config.yml` comment out `# baseurl: /report` -->
